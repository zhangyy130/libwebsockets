...
unsigned short tok_map[] = {
	...
	F_RSA |				F_B64U | JWK_RSA_KEYEL_E,
	F_RSA |				F_B64U | JWK_RSA_KEYEL_N,
	F_RSA | F_EC |			F_B64U | JWK_RSA_KEYEL_D,
	F_RSA |				F_B64U | JWK_RSA_KEYEL_P,
	F_RSA |				F_B64U | JWK_RSA_KEYEL_Q,
	F_RSA |				F_B64U | JWK_RSA_KEYEL_DP,
	F_RSA |				F_B64U | JWK_RSA_KEYEL_DQ,
	F_RSA |				F_B64U | JWK_RSA_KEYEL_QI,

	F_RSA | F_EC | F_OCT | F_META |		 JWK_META_KTY,
		       F_OCT |		F_B64U | JWK_OCT_KEYEL_K,

		F_EC |				 JWK_EC_KEYEL_CRV,
		F_EC |			F_B64U | JWK_EC_KEYEL_X,
		F_EC |			F_B64U | JWK_EC_KEYEL_Y,
...
lws_jwk_dump(struct lws_jwk *s)
{
	...
	switch (s->kty) {
	default:
	case LWS_JWK_KYT_UNKNOWN:
		lwsl_err("%s: jwk %p: unknown type\n", __func__, s);

		return 1;
	case LWS_JWK_KYT_OCT:
		elems = LWS_COUNT_OCT_KEY_ELEMENTS;
		enames = oct_names;
		b64 = oct_b64;
		break;
	case LWS_JWK_KYT_RSA:
		elems = LWS_COUNT_RSA_KEY_ELEMENTS;
		enames = rsa_names;
		b64 = rsa_b64;
		break;
	case LWS_JWK_KYT_EC:
		elems = LWS_COUNT_EC_KEY_ELEMENTS;
		enames = ec_names;
		b64 = ec_b64;
		break;
	}
...
static int
_lws_jwk_set_element_jwk_b64u(struct lws_jwk_elements *e, char *in, int len)
{
	int dec_size = ((len * 3) / 4) + 4, n;
