...
{
	struct lws_genecdh_ctx ctx;
	struct lws_jwk jwk;
	struct lws_jwk_elements el[LWS_COUNT_EC_KEY_ELEMENTS];
	...
	lws_genec_dump(el);
	lws_jwk_destroy_genec_elements(el);

	lws_genecdh_destroy(&ctx);
